...
**Misión de este Documento:** Describir la arquitectura y los mecanismos de la capa responsable de la interacción directa con la GUI externa. En la `v0.8.0` del `Praxis Heuristic Engine`, esta capa es el conjunto de **"manos y ojos"** del motor, diseñada para operar de forma fiable en un entorno "ciego" y multiplataforma.
...
## 2. Arquitectura: La Fachada de Control Remoto Multiplataforma
...
Este componente actúa como la **Capa de Abstracción de Interfaz (IAL)** del motor. Su propósito es traducir los comandos abstractos del resto del sistema (ej. "enviar estas teclas") en acciones concretas del sistema operativo, aislando al motor de los detalles de implementación de la librería subyacente, que es **`pywinauto` para Windows y `xdotool` para Linux**.
...
Para operar de forma fiable a pesar de ser "ciego", el motor no confía en la suerte. La `RemoteControlFacade` implementa protocolos de resiliencia que son fundamentales para su éxito.
...
*   **La Solución:** El método privado `_ensure_focus()` se invoca **antes de cada acción crítica** (`type_keys`, `read_clipboard...`). Este método verifica si la ventana objetivo sigue siendo la ventana activa. Si no lo es, intenta activamente traerla de vuelta al frente antes de proceder. Si no puede recuperar el foco, lanza una `FocusError`, un evento que el motor puede gestionar.
...
### 3.3. Protocolo de Errores Resilientes

La comunicación con la GUI es inherentemente frágil. Para gestionar esto, todas las excepciones técnicas lanzadas por la fachada (como `FocusError` o `ClipboardError`) heredan de una clase base `AutomationError`. Esta clase base incluye metadatos críticos que informan una estrategia de manejo de errores inteligente:

*   **`is_retryable` (bool):** Informa a las capas superiores si el error es potencialmente transitorio (ej. una pérdida temporal de foco) y la operación puede ser reintentada de forma segura.
*   **`error_code` (str):** Proporciona un identificador único para el error, facilitando el logging, la monitorización y la creación de alertas específicas.

Este diseño transforma los errores de simples finales de ejecución a eventos informados que el orquestador del motor puede gestionar de forma inteligente.
...
| :--- | :--- | :--- |
| **`find_and_focus_window(title)`**| Localiza la ventana de destino por su título y la activa. Es el punto de entrada de la interacción en Windows y Linux. | Acción |
...
3.  **Navegación Externalizada:** La fachada es un ejecutor "tonto". No tiene conocimiento de la estructura de la GUI. La inteligencia para decidir *qué* teclas enviar (ej. `"{TAB 2}"`) es **cargada por los `Handlers` desde archivos de configuración `.ini`**. Esto ofrece una gran flexibilidad, pero dispersa la lógica de flujo de la aplicación entre el código y los archivos de configuración, lo que puede dificultar el seguimiento de un proceso completo.
